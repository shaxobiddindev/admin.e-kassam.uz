  ...
  return (
    <>
      <button type="button"
        ...
        {activeCount > 0 && <span className="flt-btn__badge">{activeCount}</span>}
      </button>
      ...
      {open && (
        /* ⚠ ADMINDA `Modal` — u allaqachon `document.body` ga portal
           qiladi (ilovadagi `Overlay` ning aynan shu vazifasi). Ikkinchi
           qatlam yozish bir xil xatoni ikki joyda saqlash bo'lardi. */
        <Modal title={t("filter.title")} size="md" onClose={() => setOpen(false)}
               footer={<>
