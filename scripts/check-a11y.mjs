...
for (const theme of ["light", "dark"]) {
  ...
  page.on("request", (req) => {
    ...
    /* ⚠ Profil obyekt qaytarishi SHART: bo'sh ro'yxat berilganda
       `useAuth` uni tushunmay, panel kirish ekranida qolib ketardi va
       a11y aslida BOSHQA sahifani tekshirardi. */
    const data = new URL(req.url()).pathname.endsWith("/auth/admin/me")
      ? { id: 1, username: "a11y", fullName: "A11y", role: "SUPER_ADMIN", permissions: null }
      : [];
  ...
  await page.evaluateOnNewDocument(() => {
    ...
    localStorage.setItem("ek_refresh", "a11y");
    localStorage.setItem("ek_type", "admin");
    localStorage.setItem("ek_role", "SUPER_ADMIN");
